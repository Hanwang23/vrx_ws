...
def simulation(world_name, headless=False, paused=False, extra_gz_args=''):
    ...
    gz_sim = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(
            ...
            '/gz_sim.launch.py']),
        # VRX is built against Gazebo Garden / gz-sim7.  ros_gz_sim's Humble
        # launch file defaults to gz_version=6 (Fortress / ignition-gazebo6),
        # which cannot load gz-sim7 plugins.
        launch_arguments={
            'gz_args': ' '.join(gz_args),
            'gz_version': '7',
        }.items())

    # Register handler for shutting down ros launch when gz sim exits
    # monitor_sim.py will run until it can not find the gz sim process.
